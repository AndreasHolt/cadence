...
func main() {
	...
	for _, taskList := range cfg.TaskLists {
	...
	wg.Add(1)
	go runGenerator(ctx, &wg, frontend, cfg.Domain, cfg.TaskLists, cfg.Generator.RatePerSecond, st)
...
func runGenerator(
	...
	for {
		...
		case <-ticker.C:
			...
			_, err := frontend.StartWorkflowExecution(reqCtx, &types.StartWorkflowExecutionRequest{
				Domain:     domainName,
				WorkflowID: fmt.Sprintf("kind-lab-workflow-%d", currentScheduleID),
				WorkflowType: &types.WorkflowType{
					Name: "kind-lab-benchmark",
				},
				TaskList: &types.TaskList{
					Name: taskList.cfg.Name,
