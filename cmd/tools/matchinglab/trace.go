...
func buildTraceWorkload(cfg traceConfig) (*workload, error) {
	...
	events := makeTraceEvents(rows, columns, cfg)
	if len(events) == 0 {
...
func makeTraceEvents(rows [][]float64, columns []traceColumn, cfg traceConfig) []traceEvent {
	...
	for rowIndex, row := range rows {
		...
		for _, column := range columns {
			...
			for eventIndex := range eventCount {
				workflowID := fmt.Sprintf("trace-row-%06d-tl-%04d-seq-%06d", rowIndex, column.index, eventIndex)
