...
The trace replay scenario is:

- `environment/kind-lab/scenarios/trace-21-12.yaml`

It replays `21-12_00-06_fixed.csv` as deterministic per-task-list start times. Each CSV column becomes a task list, each CSV row is a load window, and the number of workflow starts in that window is:

```text
round(csv_qps * qps_scale * interval_seconds)
```

Events are spread evenly across the window and sorted by time, task list, and workflow ID. This gives each algorithm branch the same input schedule. The default trace scenario uses `qps_scale: 0.25`, `top_n: 100`, and the first 180 rows.
