...
GREEDY_MOVE_PENALTY_COEFFICIENT="${GREEDY_MOVE_PENALTY_COEFFICIENT:-0.4}"
...
case "$GREEDY_ENABLE_MULTI_MOVE" in
  true|false)
    ;;
  *)
    echo "GREEDY_ENABLE_MULTI_MOVE must be true or false" >&2
    exit 2
    ;;
esac
...
case "$MATCHING_ENABLE_ADAPTIVE_SCALER" in
  true|false)
    ;;
  *)
    echo "MATCHING_ENABLE_ADAPTIVE_SCALER must be true or false" >&2
    exit 2
    ;;
esac

if ! [[ "$MATCHING_NUM_TASKLIST_READ_PARTITIONS" =~ ^[1-9][0-9]*$ ]]; then
  echo "MATCHING_NUM_TASKLIST_READ_PARTITIONS must be a positive integer" >&2
  exit 2
fi
...
awk -v heterogeneity_mode="$GREEDY_HETEROGENEITY_MODE" -v move_scoring_mode="$GREEDY_MOVE_SCORING_MODE" -v move_penalty_coefficient="$GREEDY_MOVE_PENALTY_COEFFICIENT" -v cpu_smoothing_tau="$GREEDY_CPU_SECONDS_SMOOTHING_TAU" -v enable_swap="$GREEDY_ENABLE_SWAP" -v enable_multi_move="$GREEDY_ENABLE_MULTI_MOVE" -v matching_enable_adaptive_scaler="$MATCHING_ENABLE_ADAPTIVE_SCALER" -v matching_read_partitions="$MATCHING_NUM_TASKLIST_READ_PARTITIONS" -v matching_write_partitions="$MATCHING_NUM_TASKLIST_WRITE_PARTITIONS" '
  ...
  $0 == "shardDistributor.loadBalancingGreedy.enableMultiMove:" {
    in_enable_multi_move_key = 1
    print
    next
  }
  $0 == "matching.enableAdaptiveScaler:" {
    in_matching_enable_adaptive_scaler_key = 1
    print
    next
  }
  $0 == "matching.numTasklistReadPartitions:" {
    in_matching_read_partitions_key = 1
    print
    next
  }
  $0 == "matching.numTasklistWritePartitions:" {
    in_matching_write_partitions_key = 1
    print
    next
  }
  ...
  in_enable_multi_move_key && $1 == "-" && $2 == "value:" {
    print "  - value: " enable_multi_move
    in_enable_multi_move_key = 0
    next
  }
  ...
  in_matching_read_partitions_key && $1 == "-" && $2 == "value:" {
    print "  - value: " matching_read_partitions
    in_matching_read_partitions_key = 0
    next
  }
  in_matching_write_partitions_key && $1 == "-" && $2 == "value:" {
    print "  - value: " matching_write_partitions
    in_matching_write_partitions_key = 0
    next
  }
  { print }
...
GREEDY_ENABLE_MULTI_MOVE="$GREEDY_ENABLE_MULTI_MOVE" \
...
MATCHING_A_CPU="$MATCHING_A_CPU" MATCHING_B_CPU="$MATCHING_B_CPU" MATCHING_C_CPU="$MATCHING_C_CPU" \
MATCHING_A_BURN="$MATCHING_A_BURN" MATCHING_B_BURN="$MATCHING_B_BURN" MATCHING_C_BURN="$MATCHING_C_BURN" \
