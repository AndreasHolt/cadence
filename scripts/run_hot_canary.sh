...
BASE_CANARY_GRPC_PORT=${BASE_CANARY_GRPC_PORT:-7953}

echo "Launching $N canary processes with follwing hot-load settings:"
for i in $(seq 1 "$N"); do
  port=$((BASE_CANARY_GRPC_PORT + i - 1))
  scale=$(python3 - <<PY
...
PY
  ...
  echo "  proc $i: SD_EXEC_LOAD_SCALE=$scale port=$port"
  (
    export SD_EXEC_LOAD_SCALE="$scale"
    exec $CMD --canary-grpc-port "$port" >"/tmp/hot-canary-$i.log" 2>&1 &
  )
  sleep 1
done
