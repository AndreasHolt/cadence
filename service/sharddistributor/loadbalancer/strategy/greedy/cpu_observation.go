...
func updateExecutorPressureCostObservations(state *store.NamespaceState, loads map[string]float64) map[string]float64 {
	costs := make(map[string]float64)
	for executorID, executor := range state.Executors {
		cost, ok := computeExecutorPressureCostObservation(executor.Metadata, loads[executorID])
		if ok {
			costs[executorID] = cost
		}
	}
...
func computeExecutorPressureCostObservation(metadata map[string]string, load float64) (float64, bool) {
