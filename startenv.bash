...
SHARD_DISTRIBUTOR_REPLAY_SPEED="${SHARD_DISTRIBUTOR_REPLAY_SPEED:-$CANARY_REPLAY_SPEED}"
...
start_cadence_server_detached() {
  if [[ "$SKIP_BUILD" != "1" ]]; then
    ...
    echo "==> SKIP_BUILD=1, skipping cadence/cadence-server build"
  fi

  echo "==> Starting cadence-server (detached; keeps running)"
  echo "    SHARD_DISTRIBUTOR_REPLAY_SPEED=$SHARD_DISTRIBUTOR_REPLAY_SPEED"
  : > "$CADENCE_LOG"

  # Detach so it survives script exit / Ctrl+C and keeps logging
  SHARD_DISTRIBUTOR_REPLAY_SPEED="$SHARD_DISTRIBUTOR_REPLAY_SPEED" \
...
start_canary_foreground() {
  if [[ "$SKIP_BUILD" != "1" ]]; then
    ...
    echo "==> SKIP_BUILD=1, skipping sharddistributor-canary build"
  fi
...
case "$cmd" in
  start)
    ...
    if [[ "$SKIP_DOCKER_RESTART" != "1" ]]; then
      ...
      echo "==> SKIP_DOCKER_RESTART=1, leaving prometheus/grafana as-is"
    fi
    start_cadence_server_detached
    start_canary_foreground
    ;;
